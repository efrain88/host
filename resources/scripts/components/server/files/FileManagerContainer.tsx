...
export default () => {
    ...
    const baseFiltered = isTrash ? (files || []) : (files?.filter((file) => file.name !== '.trash') || []);
    const filteredFiles = baseFiltered.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase()));
        ...
        <ServerContentBlock title={'Administrador de Archivos'} showFlashKey={'files'}>
            <ErrorBoundary>
                ...
                <div className="flex items-center gap-3 mb-6 overflow-x-auto no-scrollbar pb-1">
                    ...
                    {/* Vaciar Papelera button (only in trash) */}
                    {isTrash && (
                        <button
                            onClick={emptyTrash}
                            disabled={filteredFiles.length === 0 || isEmptyingTrash}
                            className="flex items-center gap-x-2 bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold h-[42px] px-4 rounded-xl transition-all duration-200 shadow-lg text-xs whitespace-nowrap shrink-0"
                        >
                            <FontAwesomeIcon icon={faTrashAlt} />
                            <span className="hidden lg:inline">Vaciar Papelera</span>
                        </button>
                    )}
